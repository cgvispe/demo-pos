...
### System
- Sesami-inspired dark theme by default (teal `#00c4b3` accent)
...
└── server/                   # Node.js + Express backend
    ...
    │   └── sesami.js          # RC5000 proxy: payin, poll, finish, cancel
    └── sesami/
...
| `port` | `3000` for HTTP, `4443` for HTTPS |
| `useHttps` | `true` if using HTTPS (self-signed cert is accepted) |
...
Log in as `admin` → **Manager Panel → Settings → RC5000 Connection** → fill in the fields → **Save Changes**. Use **Test Connection** to verify reachability.
...
- Header and payload encoded with **base64url** (not base64)
...
- ⚠️ The device allows **only one active session at a time**. Always call `POST /logout` when done, even on error.
...
GET /api/sesami/operation/:id  (every 1.5 seconds)
  ...
  │  status 4/5  → SUCCESS ──────┼──► POST /operation/finish → logout → save tx
  │  status 7    → NO CHANGE ────┼──► Cashier decides: Finish or Cancel
  │  status 3/9  → CANCELLED ────┼──► POST /operation/cancel → logout
  │  status 6    → ERROR ────────┼──► logout
  └───────────────────────────────┘
```

**Critical:** All amounts in the RC5000 API are in **cents (integer)**. €20.00 = `2000`.
...
| `3` | Cancelled | Show cancelled screen, logout |
| `4` | Finished | Show success, call finish, logout |
| `5` | Finished by system | Show success, call finish, logout |
| `6` | Error | Show error screen, logout |
| `7` | Amount not available (insufficient change) | Cashier chooses: Finish or Cancel |
| `8` | Finished incomplete | Treat as success |
| `9` | Cancelled incomplete | Treat as cancelled |
...
| POST | `/sesami/payin` | ✓ | Login + start PayIn Amount operation |
| GET | `/sesami/operation/:id` | ✓ | Poll operation status |
| POST | `/sesami/operation/finish` | ✓ | Finish operation, save transaction, logout |
| POST | `/sesami/operation/cancel` | ✓ | Cancel operation, logout |
| GET | `/sesami/transactions` | ✓ | Transaction history |
...
- Code regenerates automatically when the category is changed (reclassification)
...
- **RC5000**: IP, port, POS ID, username, secret key, HTTPS toggle + live connection test
